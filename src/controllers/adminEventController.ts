...
export const listEvents = async (req: Request, res: Response) => {
    try {
        ...
        const sortOrder: Record<string, 1 | -1> = sort === 'endDate' ? { endDate: 1 } : { startDate: 1 };

        const [events, total] = await Promise.all([
            Event.find({}).sort(sortOrder).skip(skip).limit(limitNum).lean(),
...
export const updateEvent = async (req: Request, res: Response) => {
    try {
        ...
        if (shortDescription !== undefined) updates.shortDescription = typeof shortDescription === 'string' ? shortDescription.trim() || null : shortDescription;
        if (longDescription !== undefined) updates.longDescription = typeof longDescription === 'string' ? longDescription.trim() || null : longDescription;
        if (imageUrl !== undefined) updates.imageUrl = typeof imageUrl === 'string' ? imageUrl.trim() || null : imageUrl;
        if (venue !== undefined) updates.venue = typeof venue === 'string' ? venue.trim() || null : venue;
        ...
        const finalStart = (updates.startDate as Date) ?? existing.startDate;
        const finalEnd = (updates.endDate as Date) ?? existing.endDate;
